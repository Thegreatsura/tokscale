...
async function loadWrappedData(options: WrappedOptions): Promise<WrappedData> {
  ...
  const sources = options.sources || ["opencode", "claude", "codex", "gemini", "cursor", "amp", "droid", "openclaw", "pi", "kimi", "roocode", "kilocode"];
  const localSources = sources.filter(s => s !== "cursor") as ("opencode" | "claude" | "codex" | "gemini" | "amp" | "droid" | "openclaw" | "pi" | "kimi" | "roocode" | "kilocode")[];
  ...
  const phase1Results = await Promise.allSettled([
    ...
    localSources.length > 0
      ...
      : Promise.resolve({ messages: [], opencodeCount: 0, claudeCount: 0, codexCount: 0, geminiCount: 0, ampCount: 0, droidCount: 0, openclawCount: 0, piCount: 0, kimiCount: 0, roocodeCount: 0, kilocodeCount: 0, processingTimeMs: 0 } as ParsedMessages),
  ]);
