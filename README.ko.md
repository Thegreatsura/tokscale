...
| <img width="48px" src="https://avatars.githubusercontent.com/u/189203002?s=200&v=4" alt="Codebuff" /> | [Codebuff](https://codebuff.com/) | `~/.config/manicode/` (+ `manicode-dev`, `manicode-staging`; `CODEBUFF_DATA_DIR`로 오버라이드 가능) | ✅ 지원 |
...
Antigravity 동기화는 Antigravity가 활성화된 에디터가 실행 중이고 로컬 언어 서버를 사용할 수 있을 때만 동작합니다. tokscale은 해당 로컬 언어 서버에서 사용량을 읽어 정규화된 아티팩트를 로컬에 캐시합니다.
...
| Antigravity | `~/.config/tokscale/antigravity-cache/sessions/` | `%USERPROFILE%\.config\tokscale\antigravity-cache\sessions\` | 에디터 실행 중에 `tokscale antigravity sync`가 채움 |
